          ...
          currentAjaxRequest = $.getJSON(searchURL +'*&view=json', function(data) {
            ...
            if(data.results_count == 0) {
              ...
              if(data.results_count >= 5) {
                resultsList.append('<li class="all-results"><span class="title see-all"><a href="' + searchURL + '*">' + "translation missing: tr.general.search.all_results" + ' (' + data.results_count + ')</a></span></li>');
              }
    ...
    $('.js-savings').html('');
      ...
      $.each( product.collection_handles, function( value, index ) {
        if (this.toString() == 'new'){
          $('.js-new-banner').html("Yeni");
        }
        if (this.toString() == 'pre-order'){
          $('.js-pre-order-banner').html("Ön-Sipariş");
        }
        if (this.toString() == 'coming-soon'){
          product.price = "Yakında";
        }
      ...
      if (product.single_variant == 'true'){
        ...
        if (product.available == true) {
          ...
          if (product.compare_at_price > product.price) {
            ...
            $('.js-savings').html("Kârınız" + ' ' + parseInt(((product.variants[0].compare_at_price - product.variants[0].price) * 100) / product.variants[0].compare_at_price) + '% (' + '<span class="money">' + Shopify.formatMoney(product.variants[0].compare_at_price - product.variants[0].price, product.money_format) + '</span>)');
          }

          if (product.price == "Yakında"){
            $('.js-current-price').html("Yakında");
          ...
          if (variant == product.variants[i].id.toString()){
            ...
            if (product.variants[i].available == true) {
              ...
              if (product.variants[i].compare_at_price > product.variants[i].price) {
                ...
                $('.js-savings').html("Kârınız" + ' ' + parseInt(((product.variants[i].compare_at_price - product.variants[i].price) * 100) / product.variants[i].compare_at_price) + '% (' + '<span class="money">' + Shopify.formatMoney(product.variants[i].compare_at_price - product.variants[i].price, product.money_format) + '</span>)');
              }
