...
selectCallback = function(variant, selector) {
  ...
  var notifyEmail = "Eposta adresini gir...";
  var notifyEmailValue = "";
  var notifySend = "Gönder";

  var notifyUrl = $notifyFormInputs.data('url');

  if (variant) {
    var notifyMessage = "translation missing: tr.products.notify_form.message_content_first"+variant.title+"translation missing: tr.products.notify_form.message_content_last"+notifyUrl;
  }
  ...
  if (variant && variant.available == true) {
    if(variant.price < variant.compare_at_price){
      ...
      $('.savings', $product).html("Kârınız" + ' ' + parseInt(((variant.compare_at_price - variant.price) * 100) / variant.compare_at_price) + '% (' + '<span class="money">' + Shopify.formatMoney(variant.compare_at_price - variant.price, $('body').data('money-format')) + '</span>)');
    ...
    var message = variant ? "Tükendi" : "Tedarik Edilememektedir";
...
var productPage = {
  init: function(){
    ...
    if (window.location.search === '?contact_posted=true') {
      ...
      $('.notify_form .contact-form').prev('.message').html("Teşekkürler! Stoklar yenilendiğinde seni bilgilendireceğiz!");
    }
